...
export default function ArraysPage() {
  ...
  if (!state.tier) return <main className="mx-auto w-full max-w-3xl space-y-4 p-8"><h1 className="text-3xl font-semibold">Start with your Tier</h1><Link className="underline" href="/onboarding">Complete onboarding to open Arrays</Link></main>;
  if (state.topics.arrays.status === "locked") return <main className="p-8">Arrays is locked. <Link href="/" className="underline">Return to your path</Link></main>;
  const problem = arraysProblems[state.tier];
  return <ArraysRunner key={`${problem.id}:${state.language}`} problem={problem} language={state.language} />;
}

function ArraysRunner({ problem, language }: { problem: PracticeProblem; language: Language }) {
  ...
  async function run(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (outcome.results.every((result) => result.passed === true)) {
        dispatch({ type: "solveProblem", topicId: "arrays", problemId: problem.id });
      }
    } catch {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl space-y-8 px-6 py-12">
      ...
      <header className="space-y-3">
        <p className="text-sm uppercase tracking-wide">Arrays · {state.tier}</p>
      ...
      {state.topics.arrays.status === "complete" && <div role="status" className="space-y-2 rounded-xl border border-emerald-500 p-5"><p className="font-semibold">Arrays complete. Hashing is unlocked!</p><Link href="/" className="underline">Return to your path</Link></div>}
    </main>
  );
}
